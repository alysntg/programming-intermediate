...
def caesar_cipher(message, shift):
    ...
    for character in message:
        if character == " ":
            ...
            new_index = (character_index + shift) % 26
            new_message = alphabet[new_index]
            initializer += new_message
    return initializer
...
def vigenere_cipher(message, key):
    ...
    initializer = ""
    length_key = len(key)
    for i, charac_msg in enumerate(message):
        if charac_msg == " ":
            initializer += " "
        else:
            charac_key = key[i % length_key]  
            message_index = alphabet.index(charac_msg)
            key_index = alphabet.index(charac_key)
            new_index = (message_index + key_index) % 26
            new_charac = alphabet[new_index]
            initializer += new_charac
    return initializer
